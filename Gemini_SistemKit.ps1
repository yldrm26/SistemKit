...
# ================================================================
#  SISTEMKIT v8.0 - AKILLI NAVIGASYON ÜNİTESİ (LİKİT MİMARİ)
# ================================================================
...
$sekmeler = @("Sistem Bakimi","Sistem Izleme","Ag Araclari","Baslangic","Araclar","Sistem Kurulumu","Hakkinda")
$sekmeBtnleri = @()
...
for($i=0; $i -lt $sekmeler.Count; $i++){
    ...
    $b.Add_Click({
        ...
        $altDurum.Text = "SistemKit v8.0 | [$modStr2] $($sekmeler[$idx])"
    })
...
$sekmeAg        = Yeni-Sekme "Ag Araclari"
...
# ----------------------------------------------------------------
    ...
    $btnAkilliKaldiriciAc.Add_Click({
        ...
        $kForm.Text = "SistemKit v8.0 - Akilli Uygulama Kaldirici & Derin Temizlik"
...
# ================================================================
    #  SEKME 3 - AG ARACLARI (REVİZE EDİLMİŞ TASARIM)
    # ================================================================
    $sekmeAg.Controls.Add((Yeni-Etiket "Ag ve Baglanti Araclari" (New-Object System.Drawing.Font("Segoe UI",12,[System.Drawing.FontStyle]::Bold)) $FG_MAIN 15 12 600 28))
    
    # [CANLI HIZ GÖSTERGESİ] Üst sağ köşedeki anlık trafik monitörü
    $lblAgHizCanli=Yeni-Etiket "Hiz: --" (New-Object System.Drawing.Font("Segoe UI",9,[System.Drawing.FontStyle]::Bold)) $INFO 610 14 440 22
    $sekmeAg.Controls.Add($lblAgHizCanli)
    $agHizTimer=New-Object System.Windows.Forms.Timer; $agHizTimer.Interval=2000
    $agHizTimer.Add_Tick({
        try{
            $nic3=Get-WmiObject Win32_PerfRawData_Tcpip_NetworkInterface -EA SilentlyContinue|
                  Where-Object{$_.Name -notlike "*Loopback*" -and $_.BytesTotalPersec -gt 0}|Select-Object -First 1
            if($nic3){
                $ind=[math]::Round($nic3.BytesReceivedPersec/1024,1)
                $gon=[math]::Round($nic3.BytesSentPersec/1024,1)
                $iStr=if($ind -gt 1024){"$([math]::Round($ind/1024,2)) MB/s"}else{"$ind KB/s"}
                $gStr=if($gon -gt 1024){"$([math]::Round($gon/1024,2)) MB/s"}else{"$gon KB/s"}
                $lblAgHizCanli.Text="Indir: $iStr  |  Gonder: $gStr"
            }
        }catch{}
    })
    $tabs.Add_SelectedIndexChanged({
        if($tabs.SelectedTab -eq $sekmeAg){$agHizTimer.Start()}else{try{$agHizTimer.Stop()}catch{}}
    })

    # --- [KART 1] AG BILGILERI ---
    $kartAg1=Yeni-Kart 15 50 380 240; $sekmeAg.Controls.Add($kartAg1)
    $kartAg1.Controls.Add((Yeni-Etiket "Ag Bilgileri" (New-Object System.Drawing.Font("Segoe UI",9,[System.Drawing.FontStyle]::Bold)) $FG_MAIN 12 10 350 22))
    $logAg=Yeni-MetinKutusu 10 38 358 158; $kartAg1.Controls.Add($logAg)
    $btnAgBilgi=Yeni-Buton "Bilgileri Al" $ACCENT 130 30 "Yerel IP, gateway ve DNS bilgilerini goster"
    $btnAgBilgi.Location=[System.Drawing.Point]::new(10,202); $kartAg1.Controls.Add($btnAgBilgi)
    $btnAgBilgi.Add_Click({
        $logAg.Clear()
        try{
            $ads=Get-NetIPAddress -AddressFamily IPv4 -EA SilentlyContinue|Where-Object{$_.IPAddress -notlike "127.*"}
            foreach($a in $ads){
                Yaz-Log $logAg "Adaptor   : $($a.InterfaceAlias)" $INFO
                Yaz-Log $logAg "IP Adresi  : $($a.IPAddress)" $FG_MAIN
                Yaz-Log $logAg "Onek       : /$($a.PrefixLength)" $FG_MAIN
                Yaz-Log $logAg "---" $FG_DIM
            }
            $gw=(Get-NetRoute -DestinationPrefix "0.0.0.0/0" -EA SilentlyContinue|Select-Object -First 1).NextHop
            Yaz-Log $logAg "Gateway    : $gw" $SUCCESS
            ...
            Yaz-Log $logAg "DNS        : $($dns -join ', ')" $SUCCESS
        }catch{Yaz-Log $logAg "Hata: $_" $DANGER}
    })

    # --- [KART 2] PING TESTI ---
    $kartAg2=Yeni-Kart 410 50 385 240; $sekmeAg.Controls.Add($kartAg2)
    $kartAg2.Controls.Add((Yeni-Etiket "Ping Testi" (New-Object System.Drawing.Font("Segoe UI",9,[System.Drawing.FontStyle]::Bold)) $FG_MAIN 12 10 350 22))
    $kartAg2.Controls.Add((Yeni-Etiket "Hedef:" (New-Object System.Drawing.Font("Segoe UI",8.5)) $FG_DIM 12 38 50 20))
    $metnPing=New-Object System.Windows.Forms.TextBox
    $metnPing.Location=[System.Drawing.Point]::new(64,36); $metnPing.Size=[System.Drawing.Size]::new(185,22)
    $metnPing.BackColor=$BG_CARD; $metnPing.ForeColor=$FG_MAIN; $metnPing.BorderStyle="None"
    $metnPing.Font=New-Object System.Drawing.Font("Segoe UI",9.5); $metnPing.Text="8.8.8.8"
    $kartAg2.Controls.Add($metnPing)
    $btnPing=Yeni-Buton "Ping At" $ACCENT 88 26 "Hedefe ping gonder"
    $btnPing.Location=[System.Drawing.Point]::new(258,35); $kartAg2.Controls.Add($btnPing)
    $logPing=Yeni-MetinKutusu 10 66 363 120; $kartAg2.Controls.Add($logPing)
    $kartAg2.Controls.Add((Yeni-Etiket "Paket:" (New-Object System.Drawing.Font("Segoe UI",8.5)) $FG_DIM 10 196 50 20))
    $sayacPing=New-Object System.Windows.Forms.NumericUpDown
    $sayacPing.Location=[System.Drawing.Point]::new(60,194); $sayacPing.Size=[System.Drawing.Size]::new(58,22)
    $sayacPing.Minimum=1; $sayacPing.Maximum=20; $sayacPing.Value=4
    $sayacPing.BackColor=$BG_CARD; $sayacPing.ForeColor=$FG_MAIN
    $kartAg2.Controls.Add($sayacPing)
    $btnPing.Add_Click({
        $logPing.Clear(); $h=$metnPing.Text.Trim(); $n=[int]$sayacPing.Value
        Yaz-Log $logPing "Ping: $h ($n paket)" $ACCENT; $form.Refresh()
        try{
            $s=Test-Connection -ComputerName $h -Count $n -EA SilentlyContinue
            if($s){
                foreach($r in $s){
                    $ms=if($r.ResponseTime){$r.ResponseTime}else{"zaman asimi"}
                    Yaz-Log $logPing "$($r.Address) -- $ms ms" $SUCCESS
                }
                $ort=[math]::Round(($s|Where-Object{$_.ResponseTime}|Measure-Object -Property ResponseTime -Average).Average,1)
                Yaz-Log $logPing "Ortalama: $ort ms" $INFO
            }else{Yaz-Log $logPing "Yanit alinamadi!" $DANGER}
        }catch{Yaz-Log $logPing "Hata: $_" $DANGER}
    })

    # --- [KART 3] ROTA TAKIBI (DARALTILDI: X=15, W=440) ---
    $kartAg3=Yeni-Kart 15 304 440 335; $sekmeAg.Controls.Add($kartAg3)
    $kartAg3.Controls.Add((Yeni-Etiket "Rota Takibi (Tracert)" (New-Object System.Drawing.Font("Segoe UI",9,[System.Drawing.FontStyle]::Bold)) $FG_MAIN 12 10 300 22))
    $kartAg3.Controls.Add((Yeni-Etiket "Hedef:" (New-Object System.Drawing.Font("Segoe UI",8.5)) $FG_DIM 12 38 50 20))
    $metnTracert=New-Object System.Windows.Forms.TextBox
    $metnTracert.Location=[System.Drawing.Point]::new(64,36); $metnTracert.Size=[System.Drawing.Size]::new(150,22)
    $metnTracert.BackColor=$BG_CARD; $metnTracert.ForeColor=$FG_MAIN; $metnTracert.BorderStyle="None"
    $metnTracert.Font=New-Object System.Drawing.Font("Segoe UI",9.5); $metnTracert.Text="google.com"
    $kartAg3.Controls.Add($metnTracert)
    $btnTracert=Yeni-Buton "Baslat" $ACCENT 80 26 "Rota takibi baslat"
    $btnTracert.Location=[System.Drawing.Point]::new(220,34); $kartAg3.Controls.Add($btnTracert)
    $logTracert=Yeni-MetinKutusu 10 66 418 255; $kartAg3.Controls.Add($logTracert)
    
    $btnTracert.Add_Click({
        $logTracert.Clear(); $h=$metnTracert.Text.Trim()
        Yaz-Log $logTracert "Rota takibi: $h" $ACCENT; $form.Refresh()
        $btnTracert.Enabled = $false
        $script:tracertJob = Start-Job {param($t) & tracert.exe -d -h 20 $t 2>&1} -ArgumentList $h
        $script:tt = New-Object System.Windows.Forms.Timer
        $script:tt.Interval = 800
        $script:tt.Add_Tick({
            if ($null -eq $script:tracertJob) { return }
            $outData = Receive-Job $script:tracertJob
            if($outData){ foreach($satir in $outData){ Yaz-Log $logTracert $satir $FG_MAIN } }
            if($script:tracertJob.State -ne "Running"){
                $script:tt.Stop()
                $finalData = Receive-Job $script:tracertJob
                if($finalData){ foreach($satir in $finalData){ Yaz-Log $logTracert $satir $FG_MAIN } }
                Remove-Job $script:tracertJob -Force -EA SilentlyContinue
                $script:tracertJob = $null
                Yaz-Log $logTracert "Tamamlandi." $SUCCESS
                $btnTracert.Enabled = $true
                $script:tt.Dispose()
            }
        })
        $script:tt.Start()
...
# --- [KART 4] PROFESYONEL AG ANALIZ PANELI (HATA TEMIZLEMELI) ---
    $kartAgHiz_Ozel = Yeni-Kart 465 304 330 335; $sekmeAg.Controls.Add($kartAgHiz_Ozel)
    $kartAgHiz_Ozel.Controls.Add((Yeni-Etiket "Coklu Hiz Testi (HTTP/S)" (New-Object System.Drawing.Font("Segoe UI",9,[System.Drawing.FontStyle]::Bold)) $FG_MAIN 12 10 250 22))

    # 1. SUNUCU VERILERI (Guncellendi)
    $sunucuListesi = @(
    ...
    $list_SunucuSecim = New-Object System.Windows.Forms.CheckedListBox
    $list_SunucuSecim.Location = [System.Drawing.Point]::new(12, 38); $list_SunucuSecim.Size = [System.Drawing.Size]::new(306, 75)
    $list_SunucuSecim.BackColor = $BG_CARD; $list_SunucuSecim.ForeColor = $FG_MAIN
    $list_SunucuSecim.BorderStyle = "None"; $list_SunucuSecim.CheckOnClick = $true
    foreach($s in $sunucuListesi){ [void]$list_SunucuSecim.Items.Add($s.Ad, $true) }
    $kartAgHiz_Ozel.Controls.Add($list_SunucuSecim)

    $btn_AnalizBaslat = Yeni-Buton "Analizi Baslat" $ACCENT 150 30
    $btn_AnalizBaslat.Location = [System.Drawing.Point]::new(90, 120); $kartAgHiz_Ozel.Controls.Add($btn_AnalizBaslat)
    
    $log_AnalizEkran = Yeni-MetinKutusu 10 160 308 130; $kartAgHiz_Ozel.Controls.Add($log_AnalizEkran)
    $lbl_AnalizOrtalama = Yeni-Etiket "Hazir" (New-Object System.Drawing.Font("Segoe UI",9,[System.Drawing.FontStyle]::Bold)) $FG_DIM 12 300 300 22
    $kartAgHiz_Ozel.Controls.Add($lbl_AnalizOrtalama)

    # 2. AKSİYON BLOĞU
    $btn_AnalizBaslat.Add_Click({
        if ($list_SunucuSecim.CheckedItems.Count -eq 0) {
            $log_AnalizEkran.Clear(); Yaz-Log $log_AnalizEkran "HATA: Secim yapilmadi!" $DANGER
            $log_AnalizEkran.Clear(); Yaz-Log $log_AnalizEkran "HATA: Secim yapilmadi!" $DANGER
            return
        }

        $this.Enabled = $false; $log_AnalizEkran.Clear()
        $toplam_M = 0; $basarili_C = 0
        $lbl_AnalizOrtalama.Text = "Analiz yapiliyor..."
        $lbl_AnalizOrtalama.ForeColor = $WARNING
        
        [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12 -bor [Net.SecurityProtocolType]::Tls13
        [Net.ServicePointManager]::ServerCertificateValidationCallback = {$true}

        foreach($item in $list_SunucuSecim.CheckedItems) {
            $cName = $item.ToString()
            $cUrl = ($sunucuListesi | Where-Object { $_.Ad -eq $cName }).Url
            
            Yaz-Log $log_AnalizEkran ">> $($cName) baglaniliyor..." $INFO
            $form.Refresh()
            ...
            try {
                ...
                $sn = [math]::Round($timer.Elapsed.TotalSeconds, 2)
                if($sn -lt 0.1){ $sn = 0.5 }
                
                $mbps = [math]::Round((($data.Length / 1MB) / $sn) * 8, 1)
                Yaz-Log $log_AnalizEkran "   Sure: $($sn) sn | Hiz: $($mbps) Mbps" $SUCCESS
                $toplam_M += $mbps; $basarili_C++
            } catch {
                Yaz-Log $log_AnalizEkran "   HATA: Baglanti kurulamadi." $DANGER
            }
            $form.Refresh()
        }
        ...
        if($basarili_C -gt 0) {
            ...
            $lbl_AnalizOrtalama.Text = "Genel Ortalama: $f_ort Mbps"
            $lbl_AnalizOrtalama.ForeColor = $SUCCESS
            Yaz-Log $log_AnalizEkran "-----------------------------" $FG_DIM
            Yaz-Log $log_AnalizEkran "BITTI: $f_ort Mbps" $ACCENT
        } else {
            # Hiçbiri başarılı olmazsa "Analiz yapılıyor" yazısını siliyoruz
            $lbl_AnalizOrtalama.Text = "Analiz basarisiz oldu."
            $lbl_AnalizOrtalama.ForeColor = $DANGER
        }
        $this.Enabled = $true; $form.Refresh()
    })
	
# ================================================================
    #  SEKME 4 - BASLANGIC PROGRAMLARI
    # ================================================================
...
# --- START: GECİKMELİ BAŞLATMA MOTORU ---
	...
	# ================================================================
    #  SEKME 5 - ARACLAR (Ağ Araçları Birleştirildi & Loglar Kaldırıldı)
    # ================================================================
	...
	# ================================================================
    #  SEKME 6 - SISTEM KURULUMU & GÖÇ ASİSTANI (LİKİT MİMARİ v8.0)
    # ================================================================
...
# ---------------- HAKKINDA SEKMESİ (ÜRETİM KÜNYESİ) ----------------
    ...
    $lblBaslik = Yeni-Etiket "SistemKit v8.0" (New-Object System.Drawing.Font("Segoe UI", 18, [System.Drawing.FontStyle]::Bold)) $FG_MAIN 20 15 600 35
    ...
    $btnGuncelle.Add_Click({
        ...
        $form.Refresh()
        try {
            # NOT: GitHub hesabını açınca "KULLANICI_ADIN" yazan yerleri kendi kullanıcı adınla değiştireceksin.
            $versionUrl = "https://raw.githubusercontent.com/KULLANICI_ADIN/SistemKit/main/version.txt"
            $mevcutSurum = 8.0
            
            # İnternetten version.txt dosyasını oku
            [Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12
            $yeniSurumRaw = Invoke-RestMethod -Uri $versionUrl -UseBasicParsing -ErrorAction Stop
            $yeniSurum = [double]($yeniSurumRaw.Trim() -replace '\.',',') # Dil sorunu için virgül koruması
            ...
            if ($yeniSurum -gt $mevcutSurum) {
                ...
                if ($onay -eq "Yes") {
                    Start-Process "https://github.com/KULLANICI_ADIN/SistemKit/releases/latest"
                }
            } else {
            ...
            [System.Windows.Forms.MessageBox]::Show("Guncelleme sunucusuna (GitHub) baglanilamadi.", "Hata", 0, 48) | Out-Null
        }
    ...
    $altDurum.Text="SistemKit v8.0  |  $modStr"
    ...
    # --- PHOENIX MİMARİSİ: HATA YAKALAMA, BİLDİRİM VE YENİDEN BAŞLATMA ---
    Add-Type -AssemblyName System.Windows.Forms
    $hataMesaji = $_.Exception.Message
    $satir = $_.InvocationInfo.ScriptLineNumber
    
    # 1. Arka Planda Sessizce Discord'a Gönder (Webhook)
    try {
        # KENDİ DİSCORD SUNUCUNDAN ALACAĞIN WEBHOOK LİNKİNİ BURAYA YAPIŞTIR:
        $webhookUrl = "BURAYA_DISCORD_WEBHOOK_LINKIN_GELECEK"
        $osInfo = (Get-CimInstance Win32_OperatingSystem).Caption
        $psVer = $PSVersionTable.PSVersion.ToString()
        
        $jsonPayload = @{
            content = "🚨 **SistemKit Çökme Raporu** 🚨`n**İşletim Sistemi:** $osInfo`n**PS Sürümü:** v$psVer`n**Hata Satırı:** $satir`n**Hata Detayı:** `$ $hataMesaji `" "
    ...
    [System.Windows.Forms.MessageBox]::Show(
        "SistemKit beklenmeyen bir durumla karşılaştı ve kararlılığı korumak için kendini yeniden başlatacak.`n`n(Hata raporu arka planda geliştiriciye iletildi.)",
        "SistemKit - Phoenix Koruması",
        [System.Windows.Forms.MessageBoxButtons]::OK,
        [System.Windows.Forms.MessageBoxIcon]::Warning
    ) | Out-Null
    
    # 3. Küllerinden Doğ (Uygulamayı Yeniden Başlat)
    if ($host.Name -eq "ConsoleHost") {
        Start-Process powershell -ArgumentList "-WindowStyle Hidden -NoProfile -ExecutionPolicy Bypass -File `"$PSCommandPath`""
    } else {
        # Eğer EXE olarak çalışıyorsa (PS2EXE ile derlenmişse)
